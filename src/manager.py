...
class Manager:
    ...
    def get_apartment_costs(
        ...
        if month is not None and (month < MIN_MONTH or month > MAX_MONTH):
    ...
    def create_tenants_settlements(
        ...
        if (
            apartment_settlement.month < MIN_MONTH
            or apartment_settlement.month > MAX_MONTH
        ):
            msg = "Month must be between 1 and 12"
            raise ValueError(msg)
        ...
        return [
            TenantSettlement(
                ...
                total_due_pln=(
                    apartment_settlement.total_due_pln / len(tenants_in_apartment)
                ),
            )
